...
addToPlaylist(){
  ...
  if [ $playlistItemsCount = 0 ]; then
    ...
    sendRequestWithPayload $1 "$urlBaseAPI/youtube/v3/playlistItems?part=snippet" "$playlistPayload"   
  fi
}
...
mountVideoCustomProps(){
  ...
  do
    ...
    [[ $isCustomProps == false ]] && {
      ...
      defaultCustomVideoProp=$(jq -n --arg title "$videoTitle" --argjson config "$(cat "config/$folder.json")" \
      '{title: $title, description: $config.description, tags: $config.tags, custom: true, updatedThumb: false}' -c)
